...
| Robot shoots short only while driving sideways | `GetAdjustedShot`, `TurretTargeting`, drive field-relative speeds, `shot_table.json` TOF | `SwerveChassisSpeeds/Measured`, hood/flywheel setpoints, measured flywheel RPS, pose | compare stationary vs slow strafe shots and commanded setpoints |

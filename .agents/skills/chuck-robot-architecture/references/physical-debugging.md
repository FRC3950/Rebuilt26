...
| Command logic | works from one button/auto but not another | inspect bindings, default commands, requirements, named commands, selected mode |
...
| Feed trigger is held but FUEL does not move into turrets | `Indexer`, `RobotContainer`, `CrazyModeBindings`, `Turret.isFlywheelReadyForFeed()` | `Indexer/Forward Feed Requested`, `Indexer/Feeding Forward`, commanded/measured flywheel RPS | confirm both turret flywheels are within `2.5 RPS`; reverse/unjam separately to rule out a jam |

...
| Feed trigger is held but FUEL does not move into turrets | `Indexer`, `CompBindings`, `CrazyBindings`, `DemoContainer`, `Turret.isFlywheelReadyForFeed()` | `Indexer/Forward Feed Requested`, `Indexer/Feeding Forward`, `Indexer/Auto Unjam Active`, `Indexer/Hotdog Stall Current Exceeded`, commanded/measured flywheel RPS, applied code/binding mode | confirm both turret flywheels are within `2.5 RPS`; check whether hotdog stall current is triggering auto-unjam; reverse/unjam separately to rule out a jam |
